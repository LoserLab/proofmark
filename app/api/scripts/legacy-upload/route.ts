...
export async function POST(req: Request) {
  ...
  const { data: script, error: scriptErr } = await supabase
    ...
    .insert({
      ...
      draft_label: draftLabel ?? "First Draft",
      authors_count: authorsCount ?? 1,
      work_made_for_hire: workMadeForHire ?? "no",
      publication_status: publicationStatus ?? "unpublished",
      preexisting_material: preexistingMaterial ?? "no",
      protection_path: protectionPath ?? "packet_plus_us",
    })
    .select("*")
    .single();

  if (scriptErr) {
    console.error("[api/scripts/legacy-upload] Script insert error:", scriptErr.message);
  ...
  if (verErr) {
    console.error("[api/scripts/legacy-upload] Version insert error:", {
  ...
  if (signErr) {
    console.error("[api/scripts/legacy-upload] Signed URL error:", signErr.message);
