import { NextRequest, NextResponse } from "next/server";
import crypto from "crypto";
import { supabaseServerService } from "@/lib/supabase/server";
import { sha256Hex } from "@/lib/crypto";
import { authenticateApiKey } from "@/lib/v1/auth";
import {
  invalidRequest,
  notFound,
  unsupportedFileType,
  fileTooLarge,
  internalError,
} from "@/lib/v1/errors";
import {
  isValidUuid,
  isValidMimeType,
  validateBase64File,
  sanitizeFilename,
} from "@/lib/v1/validation";
import { config } from "@/lib/config";
import { buildReceiptText } from "@/lib/receipt";
import { registerHashOnChain } from "@/lib/blockchain/register";
import { isBlockchainEnabled } from "@/lib/blockchain/client";
import type { ProtectResponse } from "@/lib/v1/types";

export async function POST(
  req: NextRequest,
  { params }: { params: { scriptId: string } }
) {
  const auth = await authenticateApiKey(req);
  if (auth instanceof NextResponse) return auth;
  const { userId } = auth;

  if (!isValidUuid(params.scriptId)) {
    return invalidRequest("Invalid scriptId format");
  }

  let body;
  try {
    body = await req.json();
  } catch {
    return invalidRequest("Invalid JSON body");
  }

  const { fileBase64, filename, mimeType } = body;

  if (!fileBase64 || !filename || !mimeType) {
    return invalidRequest("Missing required fields: fileBase64, filename, mimeType");
  }

  if (!isValidMimeType(mimeType)) {
    return unsupportedFileType();
  }

  const fileResult = validateBase64File(fileBase64);
  if (!fileResult.valid) {
    if (fileResult.error.includes("too large")) return fileTooLarge();
    return invalidRequest(fileResult.error);
  }
  const fileBuffer = fileResult.buffer;

  const supabase = supabaseServerService();

  // Verify script ownership
  const { data: script, error: sErr } = await supabase
    .from("scripts")
    .select("id, title, work_type")
    .eq("id", params.scriptId)
    .eq("user_id", userId)
    .single();

  if (sErr || !script) return notFound("Script not found");

  // Get next version number
  const { data: maxRow } = await supabase
    .from("script_versions")
    .select("version_number")
    .eq("script_id", script.id)
    .order("version_number", { ascending: false })
    .limit(1)
    .single();

  const nextVersion = (maxRow?.version_number ?? 0) + 1;

  const safeFilename = sanitizeFilename(filename);
  const sha = await sha256Hex(fileBuffer);

  // Upload to storage
  const storagePath = `${userId}/${script.id}/${crypto.randomUUID()}-${safeFilename}`;
  const bucket = config.storage.bucket;

  const { error: uploadErr } = await supabase.storage
    .from(bucket)
    .upload(storagePath, fileBuffer, {
      contentType: mimeType,
      upsert: false,
    });

  if (uploadErr) {
    console.error("[api/v1/protect/version] Storage upload error:", uploadErr.message);
    return internalError("Failed to upload file");
  }

  // Create committed version
  const committedAt = new Date().toISOString();
  const chainEnabled = isBlockchainEnabled() && body.blockchain === true;

  const { data: version, error: verErr } = await supabase
    .from("script_versions")
    .insert({
      script_id: script.id,
      user_id: userId,
      version_number: nextVersion,
      storage_path: storagePath,
      original_filename: filename,
      mime_type: mimeType,
      byte_size: fileBuffer.byteLength,
      sha256: sha,
      committed_at: committedAt,
      status: "committed",
      ...(chainEnabled ? { chain_status: "pending" } : {}),
    })
    .select("id")
    .single();

  if (verErr || !version) {
    console.error("[api/v1/protect/version] Version insert error:", verErr?.message);
    await supabase.storage.from(bucket).remove([storagePath]);
    return internalError("Failed to create version");
  }

  const versionId = version.id;

  // Update script.updated_at
  supabase
    .from("scripts")
    .update({ notes: script.work_type ? undefined : undefined })
    .eq("id", script.id)
    .then(() => {});

  // Audit log
  supabase.from("audit_log").insert({
    user_id: userId,
    script_id: script.id,
    script_version_id: versionId,
    action: "committed",
    metadata: { sha256: sha, committedAt, versionNumber: nextVersion, source: "api_v1" },
  }).then(() => {});

  // Build receipt
  const receiptText = buildReceiptText({
    title: script.title ?? "Untitled",
    draftId: script.id,
    versionId,
    filename,
    byteSize: fileBuffer.byteLength,
    sha256: sha,
    timestampUtc: committedAt,
  });

  // Fire-and-forget blockchain
  if (chainEnabled) {
    registerHashOnChain(sha, versionId, script.work_type || "other")
      .then(async (result) => {
        if (result.success && result.txHash) {
          await supabase
            .from("script_versions")
            .update({
              tx_hash: result.txHash,
              block_number: result.blockNumber,
              chain_status: "confirmed",
              chain_registered_at: new Date().toISOString(),
            })
            .eq("id", versionId);
        } else if (!result.success) {
          await supabase
            .from("script_versions")
            .update({ chain_status: "failed" })
            .eq("id", versionId);
        }
      })
      .catch(async () => {
        await supabase
          .from("script_versions")
          .update({ chain_status: "failed" })
          .eq("id", versionId);
      });
  }

  const appUrl = process.env.NEXT_PUBLIC_APP_URL || "https://proofmark.xyz";

  const response: ProtectResponse = {
    scriptId: script.id,
    versionId,
    sha256: sha,
    committedAt,
    receiptText,
    chainStatus: chainEnabled ? "pending" : null,
    badgeUrl: `${appUrl}/api/v1/badge/${sha}`,
    verifyUrl: `${appUrl}/verify?hash=${sha}`,
  };

  return NextResponse.json(response);
}
